...
def train():
    ...
    model = AutoModelForCausalLM.from_pretrained(Config.MODEL_PATH, trust_remote_code=True, torch_dtype=torch.half, device_map="auto")
...
def infer(text):
    tokenizer = AutoTokenizer.from_pretrained(Config.MODEL_PATH, use_fast=False, trust_remote_code=True)
    model = AutoModelForCausalLM.from_pretrained(Config.OUTPUT_DIR, trust_remote_code=True, torch_dtype=torch.half, device_map="auto")
